...
public class QuartzTest {
	 ...
	 * StdSchedulerFactory(); Scheduler scheduler = null; try { //
	 * 通过schedulerFactory获取一个调度器 scheduler = schedulerfactory.getScheduler();
	 * 
	 * // 创建jobDetail实例，绑定Job实现类 // 指明job的名称，所在组的名称，以及绑定job类 JobDetail jobDetail
	 * = new JobDetail("job1", "jgroup1", MyJob.class);
	 * 
	 * // 定义调度触发规则，比如每1秒运行一次，共运行8次 SimpleTrigger simpleTrigger = new
	 * SimpleTrigger("simpleTrigger", "triggerGroup"); // 马上启动
	 * simpleTrigger.setStartTime(new Date()); // 间隔时间
	 * simpleTrigger.setRepeatInterval(1000); // 运行次数
	 * simpleTrigger.setRepeatCount(8);
	 * 
	 * // 把作业和触发器注册到任务调度中 scheduler.scheduleJob(jobDetail, simpleTrigger);
	 * 
	 * // 启动调度 scheduler.start();
	 * 
	 * System.out.println(scheduler);
	 * System.out.println(simpleTrigger.getStartTime()); } catch
	...
	public static void main(String[] args) throws Exception {
		...
		// JobDetail sendScannerJob = new JobDetail("sendScannerJob", groupName,
		// MyJob.class);
		// CronTrigger trigger = new CronTrigger("sendScannerTrigger",
		// groupName, "sendScannerJob", groupName, testCron);
		...
		for (String grp : grpNames) {
			...
			// trigger = new CronTrigger("sendScannerTrigger", grp,
			// "sendScannerJob", grp, "0/15 * * * * ?");
			// sched.scheduleJob(trigger);
			System.out.println("trigger sendScannerJob, now=" + now + ", nextFireTime=" + trigger.getNextFireTime()
					+ "\n");
			// }
			System.out.println(sched.getTriggerState(TriggerKey.triggerKey("sendScannerTrigger", grp)));
		...
		Thread.sleep(4000);
		sched.deleteJob(JobKey.jobKey("sendScannerJob", "test.scanner"));
		System.out.println(sched.getTriggerState(TriggerKey.triggerKey("sendScannerTrigger", "test.scanner")));
